...
function getBaseScore(criterionConfig: any, rawScoreInput: any): number {
  if (criterionConfig.type === "A") {
    ...
    if (isNaN(score)) {
      console.warn(
        `A型小项 ${criterionConfig.name} 的分数 "${rawScoreInput}" 不是有效数字。将按0分处理。`
      );
      return 0;
    }
    return score;
    ...
    if (rawScoreInput in linguisticMap) {
      ...
      if (
        typeof rawScoreInput === "string" &&
        rawScoreInput in genericMapping
      ) {
        console.warn(
          `B型小项 ${criterionConfig.name} 使用通用评级 '${rawScoreInput}' -> ${genericMapping[rawScoreInput]}.`
        );
        return genericMapping[rawScoreInput as keyof typeof genericMapping];
      }
      console.warn(
        `B型小项 ${criterionConfig.name} 的输入 '${rawScoreInput}' 无法映射到分数。将按0分处理。`
      );
      return 0;
    }
  }
...
function checkDealBreakers(
  allTestersInputs: QuizResponse[],
  config: any,
  resultsLog: string[]
): { dealBreakerTriggered: boolean; qualityTier: string; details: string } {
        ...
        if (critConfig.crit_level === 3) {
          ...
          if (dbThreshold !== undefined && baseScore < dbThreshold) {
            ...
            const detailMsg = `评价者 ${
              ...
              critConfig.name
            } (${critId}) 给出的分数 ${baseScore.toFixed(
              2
            )} (由评级 '${rawScore}' 得到) 低于严重问题点阈值 ${dbThreshold}。`;
            resultsLog.push(`[严重问题点触发] ${detailMsg}`);
            if (!dealBreakerDetails.includes(detailMsg))
              dealBreakerDetails.push(detailMsg);
          }
        }
      }
    }
  ...
  if (dealBreakerActive) {
    return {
      ...
      qualityTier: "不可接受 (有严重问题)",
      details: dealBreakerDetails.join("\n"),
    };
  }
  return {
...
function aggregateTesterScores(allTestersInputs: QuizResponse[], config: any) {
  const aggregatedRawScores: Record<
    string,
  ...
  for (const catId in config) {
    ...
    for (const critId in config[catId].criteria) {
      ...
      aggregatedRawScores[catId][critId] = {
        score: validScoresCount > 0 ? sumScores / validScoresCount : 0,
      };
    }
  }
...
function calculateEffectiveCriterionScores(
  ...
  for (const catId in config.categories) {
    ...
    for (const critId in config.categories[catId].criteria) {
      ...
      let logEntry = `  小项 ${catId}-${critId} (${
        critConfig.name
      }): 平均原始分=${s_ij.toFixed(2)}, 影响程度=${critLevel}`;
      ...
      if (critLevel === 1) {
        ...
        if (critLow !== undefined && s_ij < critLow) {
          ...
          resultsLog.major_flaw_categories =
            resultsLog.major_flaw_categories || [];
          if (
            !resultsLog.major_flaw_categories.includes(
              config.categories[catId].name
            )
          ) {
            resultsLog.major_flaw_categories.push(
              config.categories[catId].name
            );
          }
        }
...
function calculateWeightedCategoryScores(
  effectiveScores: any,
  config: any,
  resultsLog: any
) {
  ...
  for (const catId in config.categories) {
    ...
    if (!effectiveScores[catId]) {
      resultsLog.push(
        `警告: 方面 ${catId} (${config.categories[catId].name}) 没有有效的评价小项评分数据。`
      );
    ...
    for (const critId in config.categories[catId].criteria) {
      ...
      if (!effScoreData) {
        resultsLog.push(
          `警告: 评价小项 ${catId}-${critId} (${critConfig.name}) 缺少有效评分数据。`
        );
        continue;
      }
      ...
      if (
        resultsLog.major_flaw_categories &&
        resultsLog.major_flaw_categories.includes(config.categories[catId].name)
      ) {
    ...
    if (denominator === 0) {
      s_i_eff = 0;
      resultsLog.push(
        `方面 ${config.categories[catId].name} (${catId}): 分母为0，方面得分计为0`
      );
    } else {
    ...
    if (hasCritLowTriggeredInCat && params.crit_low_cap_value !== undefined) {
      if (s_i_eff > params.crit_low_cap_value) {
        ...
        resultsLog.push(
          `  由于方面内存在关键缺陷项，方面 ${catId} 得分上限被限制为: ${s_i_eff.toFixed(
            2
          )}`
        );
      }
    ...
    resultsLog.push(
      `  方面 ${
        config.categories[catId].name
      } (${catId}) 最终有效得分: ${categoryScores[catId].toFixed(2)}`
    );
  }
...
function calculateCoreAggregatedScore(
  ...
  for (const catId in config.categories) {
    ...
    resultsLog.push(
      `  方面 ${
        config.categories[catId].name
      } (${catId}): 重要程度 ${w_i.toFixed(2)} * 得分 ${s_i_eff.toFixed(
        2
      )} = ${(w_i * s_i_eff).toFixed(2)}`
    );
  }
...
function applyAimmModule(
  ...
  if (s2_eff < 5.0 && s3_eff > 8.0) {
    ...
    flatPenalty += penalty;
    resultsLog.push(
      `  AIMM: 形状和比例(C2)得分低 (${s2_eff.toFixed(
        2
      )}) 但表面和颜色(C3)得分高 (${s3_eff.toFixed(
        2
      )})，触发冲突惩罚: -${penalty.toFixed(2)}`
    );
  } else if (s2_eff > 8.5 && s3_eff > 8.5) {
    const bonusFactor = aimmParams.synergy_bonus_c2_c3;
    globalModifier *= bonusFactor;
    resultsLog.push(
      `  AIMM: 形状和比例(C2) (${s2_eff.toFixed(
        2
      )}) 与表面和颜色(C3) (${s3_eff.toFixed(
        2
      )}) 表现均优异，触发协同奖励因子: *${bonusFactor.toFixed(2)}`
    );
  }
  ...
  if (harmonyFactorBase > 0) {
    ...
    resultsLog.push(
      `  AIMM: 整体效果和做工(C6)得分 ${s6_eff.toFixed(
        2
      )}，协调性调节因子: *${harmonyFactor.toFixed(
        3
      )} (基准分${harmonyFactorBase}, 指数${harmonyExponent})`
    );
  } else {
    resultsLog.push(
      `  AIMM: 整体效果和做工(C6)基准分为0，跳过协调性调节因子计算。`
    );
  }
  ...
  for (const catId in config.categories) {
    weightedCatScoresValues.push(
      config.categories[catId].weight * (categoryScores[catId] || 0)
    );
  }

  if (weightedCatScoresValues.length > 1) {
    const mean_wcs =
      weightedCatScoresValues.reduce((a, b) => a + b, 0) /
      weightedCatScoresValues.length;
    const variance_wcs =
      weightedCatScoresValues
        .map((x) => Math.pow(x - mean_wcs, 2))
        .reduce((a, b) => a + b, 0) / weightedCatScoresValues.length;
    const std_dev_wcs = Math.sqrt(variance_wcs);
    const consistencyPenalty =
      aimmParams.consistency_penalty_factor * std_dev_wcs;

    flatPenalty += consistencyPenalty;
    resultsLog.push(
      `  AIMM: 加权方面得分标准差 ${std_dev_wcs.toFixed(
        2
      )}，一致性惩罚: -${consistencyPenalty.toFixed(2)}`
    );
  }

  resultsLog.push(
    `  AIMM 应用前: TAS_core=${tas_core.toFixed(
      2
    )}, GlobalModifier=${globalModifier.toFixed(
      3
    )}, FlatPenalty=${flatPenalty.toFixed(2)}`
  );

  const tas_intermediate = tas_core * globalModifier - flatPenalty;
  resultsLog.push(
    `AIMM 应用后综合调整分 (TAS_intermediate): ${tas_intermediate.toFixed(2)}`
  );
...
function assignQualityTier(
  ...
  if (dealBreakerTriggered) {
    resultsLog.push(`\n整体评价等级: 不可接受 (有严重问题)`);
    return {
      finalTier: "不可接受 (有严重问题)",
      finalScore100: config.deal_breaker_score,
    };
  }
  ...
  if (
    resultsLog.major_flaw_categories &&
    resultsLog.major_flaw_categories.length > 0
  ) {
    if (
      finalTier !== "F (有缺陷)" &&
      finalTier !== "D (不足)" &&
      ...
      const tierNamesOrdered = Object.keys(config.quality_tiers).filter(
        (k) => k !== "不可接受 (有严重问题)"
      );
      tierNamesOrdered.sort(
        (a, b) =>
          config.quality_tiers[b].range[0] - config.quality_tiers[a].range[0]
      );
      ...
      try {
        ...
        if (
          currentTierIdxInOrdered !== -1 &&
          currentTierIdxInOrdered < tierNamesOrdered.length - 1
        ) {
          const originalTier = finalTier;
          finalTier = tierNamesOrdered[currentTierIdxInOrdered + 1];
          resultsLog.push(
            `  注意: 由于方面 ${resultsLog.major_flaw_categories.join(
              ", "
            )} 存在关键缺陷项，整体评价等级从 ${originalTier} 降至 ${finalTier}。`
          );
        } else if (currentTierIdxInOrdered !== -1) {
          resultsLog.push(
            `  注意: 方面 ${resultsLog.major_flaw_categories.join(
              ", "
            )} 存在关键缺陷项，但当前已是最低可评等级 ${finalTier}。`
          );
        }
      } catch (e) {
        resultsLog.push(
          `  警告: 无法在有序等级列表中找到当前等级 ${finalTier} 以进行降级处理。`
        );
      }
    }
  }
...
export function evaluateHeadShell_MultiTester(
  allTestersInputs: QuizResponse[],
  config: QuizConfig
): QuizResult {
  ...
  const {
    ...
    details: dealBreakerDetails,
  } = checkDealBreakers(allTestersInputs, config.categories, resultsLog);
  ...
  if (dealBreakerTriggered) {
    resultsLog.push(
      `最终百分制评价分: ${config.deal_breaker_score.toFixed(2)}`
    );
    if (dealBreakerDetails)
      resultsLog.push(`严重问题点详情: ${dealBreakerDetails}`);
    returnObject.final_score_100 = config.deal_breaker_score;
    returnObject.final_score_10_scale = config.deal_breaker_score / 10;
    return returnObject;
  }

  const aggregatedScores = aggregateTesterScores(
    allTestersInputs,
    config.categories
  );
  ...
  if (
    ...
    allTestersInputs.length > 0
  ) {
    resultsLog.push(
      "错误：无法聚合评价者分数，可能是因为所有评价者都没有提供有效的输入数据。"
    );
    returnObject.final_tier = "未评级 (无有效数据)";
    return returnObject;
  }

  const effectiveScores = calculateEffectiveCriterionScores(
    aggregatedScores,
    config,
    resultsLog
  );
  const categoryScores = calculateWeightedCategoryScores(
    effectiveScores,
    config,
    resultsLog
  );
  const tas_core = calculateCoreAggregatedScore(
    categoryScores,
    config,
    resultsLog
  );
  const tas_intermediate = applyAimmModule(
    tas_core,
    categoryScores,
    config,
    resultsLog
  ...
  const { finalTier, finalScore100 } = assignQualityTier(
    tas_final_100_scale,
    dealBreakerTriggered,
    resultsLog,
    config
